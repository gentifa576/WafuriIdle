...
| Combat retaliation | Whenever a combat damage step applies player damage to the current enemy, that enemy immediately retaliates for its zone-scaled `enemyAttack` value with no mitigation only if it survives that same damage step; retaliation consumes team HP in member order across living members. |
