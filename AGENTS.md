...
| Enemy content | Static enemy templates loaded from resource in non-prod and from DB cache in prod |
...
| Enemy sourcing | Load static enemy templates through sequential fetchers, resource first when available, then DB fallback. |
...
| Cooldown publish gating | During `FIGHTING`, suppress combat publishes for cooldown-only drift while cooldown remains above zero; publish when non-cooldown combat fields change or when a skill cooldown transitions to ready (`remaining <= 0`). |
...
| Skill auto use | During `FIGHTING`, each living member auto-uses their authored character skill whenever its cooldown is ready; ready skills execute in member order `3 -> 2 -> 1`, add damage on top of base team damage, and consume cooldown only after execution. Cooldown progression advances by real elapsed combat tick time, independent of the 1s damage cadence. |
...
| Combat stat derivation | Character combat stats scale from player level through the authoritative scaling rule, not raw linear growth. Strength, agility, and vitality each use the same soft-capped progression factor and then convert into combat-space `attack`, `hit`, and `hp`; `hit` currently uses `sqrt(agility)` with a minimum of `1`. Item stats use the authoritative nonlinear `softcap_surge` item curve with stronger post-24 item growth and stat-type-specific multipliers so gear stays modest early and becomes dominant later. Do not use intelligence or wisdom in combat yet, derive team DPS from living members only, and resolve enemy HP plus retaliation attack through zone-aware scaling from the selected enemy template. |
...
| Combat retaliation | Whenever a combat damage step applies player damage to the current enemy, that enemy immediately retaliates for its zone-scaled `enemyAttack` value with no mitigation only if it survives that same damage step; retaliation applies that full attack value to every living team member. |
| Combat wipe recovery | When every combat member reaches `0 HP`, combat enters `DOWN` for `30s`, then the team revives at `50%` HP and combat resumes against a fresh full-HP random enemy from that zone unless the player explicitly stops combat first. |
...
| Player progression | Players gain EXP and gold from kills and level globally based on nonlinear thresholds resolved by the scaling rule. The configured `experiencePerLevel` value anchors the level-2 threshold, currently `85`, while later levels use the validated soft-capped threshold family tuned for about `1-20` in `~2h` and level `100` in about `28d`. Kill rewards currently scale by defeated enemy zone level through reward exponent `0.37` layered on top of the main zone multiplier. |
| Zone progression | Zone progression is tracked per player and per zone through nonlinear zone level thresholds resolved by the scaling rule. The configured `killsPerLevel` value anchors the level-2 threshold, currently `9`, and each kill currently grants `16x rewardMultiplier` local progress points so active-zone progression can outrun player progression. Current zone level drives config-based scaling on the selected enemy template's base HP, retaliation attack, and softer EXP and gold reward scaling for future kills in that combat zone. |
...
| Loot config | Combat loot settings are config-driven. Base item drop rate is `1%` per kill, rarity rolls use `common 70%`, `rare 20%`, `epic 8%`, `legendary 2%`, and dropped item stats scale from generated `itemLevel` derived from the defeated enemy's zone level through the authoritative nonlinear item-scaling rule. |
| Zone model | Zones define level range, loot table, enemy ID list, and future event references; combat currently uses the default zone as its source of enemy references, randomly selects from that zone pool whenever combat enters `FIGHTING`, and executes grouped by zone. |
| Enemy model | Enemies define stable `id`, user-facing `name`, optional `image`, `baseHp`, and `attack` as static template data; zone content references enemies by `id`. |
...
| Incoming commands | `/ws/player/{playerId}` accepts `{ type: "START_COMBAT" }` to start combat on the socket-owning node and `{ type: "STOP_COMBAT" }` to end the player's active combat immediately. |
...
| Publish rule | Tick processing skips WebSocket publish when the player's state content did not change. Combat syncs also suppress repeated `DOWN` countdown-only snapshots after the initial `DOWN` publish until combat resumes or otherwise leaves `DOWN`, and suppress cooldown-only drift snapshots while cooldowns remain above zero. |
| Payload | Event payloads are player-scoped messages `{ type, playerId, snapshot }` for player-state sync, `{ type, playerId, snapshot, serverTime }` for combat-state sync, `{ type, playerId, events: [{ eventId, characterKey, skillKey, effectType, targetType, targetKey?, value?, statusKey?, durationMillis? }], serverTime }` for batched skill effect events, `{ type, playerId, zoneId, level, serverTime }` for zone level-up notifications, `{ type, playerId, offlineDurationMillis, kills, experienceGained, goldGained, playerLevel, playerLevelsGained, zoneId, zoneLevel, zoneLevelsGained, rewards, serverTime }` for offline progression summaries, and `{ type, playerId, commandType, message, serverTime }` for WebSocket command validation failures. `START_COMBAT` and `STOP_COMBAT` may return either an immediate combat-state sync payload or a command error payload on the player channel. Combat snapshots now include `enemyImage`, `enemyAttack`, `pendingReviveMillis`, and per-member `skillCooldownRemainingMillis` alongside enemy HP and team member HP. |
