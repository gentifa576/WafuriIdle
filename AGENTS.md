...
| Zone progression | Zone progression is tracked per player and per zone through kill counts and zone level thresholds, and current zone level drives config-based scaling on the selected enemy template's base HP plus softer EXP and gold reward scaling for future kills in that combat zone. |
